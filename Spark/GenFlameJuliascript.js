...
function GenFlameJuliascript( FnName, FnType, Variant, NumOpers, OperInfo, author, email )
{
    ...
    algorithm = algorithm.replace(/endWhile/, endWhileStr)
    algorithm = algorithm.replace(/merge/, mergeStr);
    algorithm = algorithm.replace(/endFun/, endFunStr);
...
{
	var returnStr = "    while size(<vec><dir><tr>, <index>) < size(<vec><tr>, <index>)";
...
function setBlockSize(FnType, NumOpers, OperInfo)
{
	...
	var returnStr = "        block_size = min(size(<vec><dir><tr>, <index>), nb_alg)";
