...
"""
function dots!(x::Union{LinearAlgebra.Transpose{T, Array{T}} where T <: Number, Array{T} where T <:Number}, y::Union{LinearAlgebra.Transpose{T, Array{T}} where T <: Number, Array{T} where T <:Number}, alpha::Array{T} where T <: Number)

    @assert length(alpha) == 1 "laff.dots!: alpha must contain one element."
